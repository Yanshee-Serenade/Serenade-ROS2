...
import ros_api
from ros_api import TrackingResult  # 导入强类型数据结构
...
def get_image_from_ros(
    ...
    try:
        ...
        print(
            f"[{timestamp}] 💾 保存处理后图像到: {image_save_path}（尺寸：{pil_image.size}，模式：{pil_image.mode}）"
        )
        print(
            f"[{timestamp}] 📊 ROS数据闭环获取统计：总接收{tracking_result.total_recv_size}字节，解析耗时{tracking_result.parse_cost_ms:.2f}ms"
        )
...
def load_model_vlm(model_path: str = MODEL_VLM_DEFAULT):
    ...
    try:
        ...
        model_vlm = AutoModelForCausalLM.from_pretrained(model_path, load_in_8bit=True)
...
def load_model_sam3(model_path=MODEL_SAM3_PATH):
    ...
    model = build_sam3_image_model(load_from_HF=False, checkpoint_path=model_path)
...
def generate_depth_map(image_path: str, target_shape: tuple[int, int]):
    ...
    depth_map_resized = cv2.resize(
        ...
        (
            target_shape[1],
            target_shape[0],
        ),  # cv2.resize参数为(w, h)，与target_shape (h, w)对应
...
def save_da3_depth_with_ros_keypoints(
    ...
    da3_depth_viz_result = visualize_depth(
        da3_depth_map, cmap="plasma"
    )  # plasma着色方案
    # Handle potential tuple return from visualize_depth
    if isinstance(da3_depth_viz_result, tuple):
        da3_depth_viz = da3_depth_viz_result[0]  # Take first element if it's a tuple
    else:
    ...
    if camera_point_cloud is not None and len(camera_point_cloud) > 0:
        ...
        orb_slam_depth = camera_point_cloud[
            :, 2
        ]  # 提取ORB-SLAM3真实深度（核心：从这里获取颜色映射依据）
        ...
        if len(valid_pixel_w) > 0:
            ...
            for idx, (w, h) in enumerate(zip(valid_pixel_w, valid_pixel_h)):
                ...
                orb_rgb = plasma_cmap(norm_depth)[
                    :3
                ]  # 获取plasma色卡对应的RGB值（0~1范围）
                orb_rgb_255 = (np.array(orb_rgb) * 255).astype(
                    np.uint8
                )  # 转换为0~255范围
    ...
    # 步骤4：保存叠加关键点后的DA3深度图
    da3_depth_keypoints_save_path = (
        f"{DA3_DEPTH_WITH_KEYPOINTS_SAVE_PREFIX}{timestamp}.png"
    )
    cv2.imwrite(da3_depth_keypoints_save_path, da3_depth_with_keypoints)
    print(
        f"[{timestamp}] 💾 叠加ROS关键点的DA3深度图保存到: {da3_depth_keypoints_save_path}"
    )
...
def generate_text_stream(text_query: str, image_path: str, timestamp: str):
    ...
    assert model_vlm is not None, "model_vlm should not be None at this point"
    assert processor is not None, "processor should not be None at this point"
    ...
    try:
        ...
        def generate_wrapper():
            # model_vlm is guaranteed to be not None here
...
def init_flask_app():
    ...
    def generate():
        ...
        try:
            ...
            print(
                f"[{timestamp}] 📏 原始图像尺寸: {image_shape}，准备生成对应深度图..."
            )
            ...
            # 6. 流式返回生成结果
            return Response(
                generate_text_stream(text_query, image_path, timestamp),
...
def main():
    ...
    try:
        ...
        print(
            f"[{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}] ❌ 程序启动失败: {str(e)}"
        )
        os._exit(1)
